...
public class CreateBuilderImpl implements CreateBuilder, BackgroundOperation<PathAndBytes>, ErrorListenerPathAndBytesable<String>
{
    ...
    public String forPath(String path) throws Exception
    {
        return forPath(path, client.getDefaultData());  // client.getDefaultData()为本机的IP地址
    }
    ...
    public String forPath(final String givenPath, byte[] data) throws Exception
    {
        ...
        // /locks/lock_01/lock-  ->  /locks/lock_01/_c_UUID-lock-
        ...
        {
            String path = protectedPathInForeground(adjustedPath, data, aclList);  //  /locks/lock_01/_c_UUID-lock-0000000000
                ...
                new Callable<String>()
                {
                    ...
                    public String call() throws Exception
                    {
                        ...
                        if ( createdPath == null )
                        {
                            try
                            {
                                createdPath = client.getZooKeeper().create(path, data, aclList, createMode, storingStat);  // 创建节点
                            }
                            catch ( KeeperException.NoNodeException e )
                            {
                                if ( createParentsIfNeeded )
                                {
                                    ZKPaths.mkdirs(client.getZooKeeper(), path, false, client.getAclProvider(), createParentsAsContainers); // 先创建父目录
                                    createdPath = client.getZooKeeper().create(path, data, acling.getAclList(path), createMode, storingStat);  // 创建临时顺序节点
                                }
                                else
                                {
                                    throw e;
